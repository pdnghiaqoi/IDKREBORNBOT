...
module.exports.run = async (bot, message, args) => {
    ...
    if(kUser == bot.user.username) return; 
    if(!kUser) return message.channel.send("Can't find user!");
    let kReason = args.join(" ").slice(22);
    if(!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send("You don't have perm to kick!");
    if(kUser.hasPermission("KICK_MEMBERS")) return message.channel.send("You can't kick him!");
    ...
    let kickChannel = message.guild.channels.find(`name`, "kick-log");
